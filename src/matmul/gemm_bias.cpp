...
int main(int argc, char *argv[]) {
  ...
  if (output_type == "fp32" && accmu_type == "fp32") {
    profile_gemm_bias<__half, __half, float, float>(manifest, m, n, k, layout_a,
                                                    layout_b, layout_c);
  } else if (output_type == "fp16" && accmu_type == "fp32") {
    profile_gemm_bias<__half, __half, __half, float>(
        manifest, m, n, k, layout_a, layout_b, layout_c);
  } else if (output_type == "fp16" && accmu_type == "fp16") {
    profile_gemm_bias<__half, __half, __half, __half>(
        manifest, m, n, k, layout_a, layout_b, layout_c);
  } else {
